        ...
        <form action="{{ route('admin.prescriptions.store') }}" method="POST">
            ...
            <div class="form-grid">
                    ...
                    <select name="medical_record_id" id="medical_record_id" class="form-control @error('medical_record_id') is-invalid @enderror" required>
                        ...
                        @foreach($records as $record)
                            <option value="{{ $record->id }}" 
                                {{ (old('medical_record_id') == $record->id || $selectedRecordId == $record->id) ? 'selected' : '' }}>
                ...
                <div class="form-group full-width">
                    ...
                    <textarea id="usage_instruction" name="usage_instruction" class="form-control @error('usage_instruction') is-invalid @enderror" placeholder="Ví dụ: Sắc uống ngày 1 thang, chia 3 lần...">{{ old('usage_instruction') }}</textarea>
                </div>

                <div class="form-group full-width">
                    <label for="general_note">Ghi chú, lời dặn thêm</label>
                    <textarea id="general_note" name="general_note" class="form-control @error('general_note') is-invalid @enderror" placeholder="Ví dụ: Kiêng đồ cay nóng, hải sản...">{{ old('general_note') }}</textarea>
                </div>
            </div>
